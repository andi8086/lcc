...
static Type xxinit(int op, char *name, Metrics m) {
	...
	switch (ty->op) {
	case INT:
		p->u.limits.max.i = ones(IR->byte_width*ty->size)>>1;
		p->u.limits.min.i = -p->u.limits.max.i - 1;
		break;
	case UNSIGNED:
		p->u.limits.max.u = ones(IR->byte_width*ty->size);
		p->u.limits.min.u = 0;
		break;
	case FLOAT:
...
#undef xx
	...
	pointersym->u.limits.max.p = (void*)ones(IR->byte_width*IR->ptrmetric.size);
