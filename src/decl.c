...
#define bits2bytes(n) (((n) + IR->byte_width - 1)/IR->byte_width)
static int regcount;
...
static void fields(Type ty) {
	  ...
	  while (istypename(t, tsym)) {
	  	...
	  	for (;;) {
			...
			if (t == ':') {
				...
				if (p->bitsize > IR->byte_width*inttype->size || p->bitsize < 0) {
					error("`%d' is an illegal bit-field size\n",
						p->bitsize);
					p->bitsize = IR->byte_width*inttype->size;
	...
	{ int bits = 0, off = 0, overflow = 0;
	  ...
	  for (p = *q; p; p = p->link) {
		...
		|| bits - 1 + p->bitsize > IR->byte_width*unsignedtype->size) {
		...
		if (p->lsb) {
			...
			if (IR->little_endian)
				...
				p->lsb = IR->byte_width*unsignedtype->size - bits + 1
					- p->bitsize + 1;
			bits += p->bitsize;
		} else
