...
#define extend(x,ty) ((x)&(1<<(IR->byte_width*(ty)->size-1)) ? (x)|((~0UL)<<(IR->byte_width*(ty)->size-1)) : (x)&ones(IR->byte_width*(ty)->size))
...
#define fieldleft(p)  (IR->byte_width*(p)->type->size - \
                        fieldsize(p) - fieldright(p))
#define fieldmask(p)  (~(fieldsize(p) < IR->byte_width*unsignedtype->size ? ~0u<<fieldsize(p) : 0u))
